...
<%
	...
	for(DataBoardVO tvo:list){
		...
		if(temp.length()>10){
			temp=temp.substring(0,10)+"..."; //10글자가 넘으면 뒤에 ... 출력
			tvo.setSubject(temp);
		}
		tvo.setSubject(temp);
	}
...
let u=0;
$(function(){
	//jquery시작 => $(document).ready(function(){})
	$('.updates').click(function(){
		$('.ups').hide(); //수정버튼 누르면 다 닫는다.
		$('.updates').text('수정');
        ...
        <table class="table">
          ...
          <tr>
            <td colspan="4" class="text-right">
              <a href="update.jsp?no=<%=vo.getNo() %>" class="btn btn-sm btn-info">수정</a>
      ...
      <div class="row">
        <div class="col-sm-8">
          <table class="table">
               	...
               	<table class="table">
               	  <tr>
               	    ...
               	    <td class="text-right">
               	      <%
               	        if(id!=null){ //로그인을 하고 ->
               	        	if(id.equals(rvo.getId())){ // 댓글을 쓴 사람이면 
               	      %>
               	  ...
               	  <tr class="ups" id="u<%=rvo.getNo() %>" style="display: none">
	                  ...
	                  <input type=submit value="댓글수정" class="btn btn-sm btn-danger" style="width: 100px;height: 90px;float: left">
	                  </form>
               	    </td>
               	  </tr>
               	</table>               
            ...
            <%if(id!=null){ %>
            <table class="table"><%-- 글쓰기 --%>
              <tr>
                <td>
                  <form method=post action="reply_insert.jsp">
                  <textarea rows=4 cols=45 name=msg style="float: left"></textarea>
                  <input type=hidden name=bno value="<%=vo.getNo()%>">
                  <input type=submit value="댓글쓰기" class="btn btn-sm btn-danger" style="width: 100px;height: 90px">
                  </form>
                </td>
              </tr>
            </table>
          ...
          <div class="col-sm-4">
            <%-- 인기 게시물 --%>
            <table class="table">
              <caption>인기 Top 10</caption> <!-- caption : 테이블의 제목 -->
              <tr>
                <th>제목</th>
                <th>이름</th>
              </tr>
              <%
              	for(DataBoardVO tvo:list) { //위에 <%에서 vo 변수명을 이용하고 있으므로 tvo로 설정
              %>
              	<tr>
