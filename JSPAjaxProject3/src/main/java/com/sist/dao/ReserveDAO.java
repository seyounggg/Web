...
public class ReserveDAO {
	...
	public String foodReserveDay(int fno) {
		...
		try {
			conn=db.getConnection();
			String sql="SELECT reserve_day FROM food_house WHERE fno=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1, fno);
			ResultSet rs=ps.executeQuery();
			rs.next();
	...
	public String reserve_day_time(int tno) {
		...
		try {
			conn=db.getConnection();
			String sql="SELECT time FROM reserve_day WHERE rno=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1, tno);
			ResultSet rs=ps.executeQuery();
			rs.next();
	...
	public String reserve_get_time(int tno) {
		...
		try {
			conn=db.getConnection();
			String sql="SELECT time FROM reserve_time WHERE tno=?";
			ps=conn.prepareStatement(sql);
			ps.setInt(1, tno);
			ResultSet rs=ps.executeQuery();
			rs.next();
