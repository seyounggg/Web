...
<%--
	...
	2) 서버
		=> Front Controller
			사용자로부터 요청을 받아서 
			모델을 찾고 
			모델에서 request/session에 데이터를 담아
			데이터를 JSP로 전송하는 역할!
		--------------------------------------
			DispatcherServlet은 배달부
		=> Model : request가 JSP 전송
			requeset.setAttribute() : 데이터를 추가해서 전송할 목적
		=> JSP에서 request에 들어가있는 데이터를 출력하기 쉽게 
			${} (EL), 자바에서 제공하는 제어문, 메소드 ...를 제공
					  -----------------------
					  태그형으로 변경 (JSTL)
		=> JSP는 스프링에서 그대로 사용!!
 --%>
...
$(function(){
	...
	$('#amount_select').change(function(){
		let amount=$(this).val();
		let price=$('#byBtn').attr("data-price");
		let total=amount*price;
		$('#total_price').html("<span style='font-color: green'>"+total+"</span>원");
		$('#price').val(price)
		$('#amount').val(amount)
		$('#cartFrm').show()
	})
...
IMP.init("imp77673478"); // 예: imp00000000
function requestPay() {
	console.log('clicked');
  ...
  <main class="container clear">
    <div class="row">
      <table class="table">
        ...
        <tr>
          <td><span style="color:green;font-size: 8pt">첫구매 할인가</span>&nbsp;<span style="color:green">${vo.goods_first_price }</span></td>
        </tr>
        <tr>
          <td>배송&nbsp;&nbsp;<span style="color:black">${vo.goods_delivery }</span></td>
        </tr>
        <tr>
          <td class="inline">
        ...
        <tr>
          <td class="inline">
            ...
            <input type=submit class="btn btn-lg btn-success" value="장바구니" style="width:200px">
            </form>
            <input type=button class="btn btn-lg btn-warning" value="바로구매" style="width:200px" data-price="${price }" id="byBtn">
          </td>
        </tr>
      </c:if>
      </table>
