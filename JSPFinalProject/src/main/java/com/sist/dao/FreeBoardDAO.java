...
public class FreeBoardDAO {
	...
	public List<FreeBoardVO> freeboardListData(int page){
		...
		try {
			...
			String sql="SELECT no,subject,name,TO_CHAR(regdate,'yyyy-MM-dd'),hit,num "
					...
					+ "FROM (SELECT /*+INDEX_DESC(project_freeboard bf_no_pk) */no,subject,name,regdate,hit "
	...
	//2. Top7(footer안에) => rownum(HIT가 많은 위에서 7개) Top-N
	//3. 글쓰기
	public void freeboardInsert(FreeBoardVO vo) {
		try {
			conn=db.getConnection();
			String sql="INSERT INTO project_freeboard "
					+ "VALUES(pf_no_seq.nextval,?,?,?,?,SYSDATE,0)";
			ps=conn.prepareStatement(sql);
