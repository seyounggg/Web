...
import com.sist.dao.*;
@WebServlet("/BoardDetailServlet") // 요 이름이 URL주소
public class BoardDetailServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 전송방식
		response.setContentType("text/html;charset=UTF-8"); //utf-8 대소문자 구분 안하지만, 대문자 권장
		// 클라이언트가 보낸 값을 받는다
		// => BoradDetailServlet?no=1
		// a.jsp?page=1 -> a.jsp가 page=1을 받아라
		String no=request.getParameter("no"); //no변수에 no를 넣어라(no=1(no))
		/*
		...
		out.println("<tr>");
		out.print("<td colspan=4 height=200 align=left align=top>");
		out.print("<pre style=\"white-space:pre-wrap\">"+vo.getContent()+"</pre>");
		// white-space:pre-wrap => 자동 줄바꿈
		// out.print("<pre>"+vo.getContent()+"</pre>");
		out.print("</td>");
		out.print("</tr>");
		
		out.print("<tr>");
		out.print("<td colspan=4 align=right>");
		out.print("<a href=BoradUpdateServlet?no="+vo.getNo()+">수정</a>&nbsp;");
		out.print("<a href=BoardDeleteServlet?no="+vo.getNo()+">삭제</a>&nbsp");
