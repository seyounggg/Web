...
public class DataCollection {
	public void foodCategoryData() {
		...
		try {
			...
			Elements title=doc.select("div.info_inner_wrap span.title"); // tag를 30개씩 모아서 읽어옴
			Elements subject=doc.select("div.info_inner_wrap p.desc"); // 30
			Elements poster=doc.select("ul.list-toplist-slider img.center-croping"); // 30
			Elements link=doc.select("ul.list-toplist-slider a"); // 30
